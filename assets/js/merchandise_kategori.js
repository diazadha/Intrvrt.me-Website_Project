...
jQuery(document).ready(function() {
    ...
    $('#table').DataTable({
        ...
        "order": [],
        "pageLength": 25,
        "ajax": { "url": base_url + "admin/merchandise/kategori_", "type": "POST" },
        "columnDefs": [{
            "targets": [-1, -2],
            "orderable": false,
    ...
    $('#table').on('click ', '.delete ', function() {
        ...
        Swal.fire({
            ...
            if (willDelete.value) {
                $.ajax({
                    url: base_url + "admin/merchandise/kategori_delete/" + id,
                    type: "POST",
                    dataType: "JSON",
    ...
    $.validator.setDefaults({
        submitHandler: function() {
            var url;
            if (save_method == 'add') {
                url = base_url + "admin/merchandise/kategori_add";
            } else {
                url = base_url + "admin/merchandise/kategori_update";
            }
            $.ajax({
                url: url,
                type: "POST",
